...
// Virtual try-on / garment transfer. Puts the garment shown in `garmentUrl`
// onto the person in `personUrl`, preserving the person's face, identity, pose
// and body — and faithfully transferring the garment's real design (logos,
// crest, sponsor, colours). This is the reference-image approach: the garment
// is COPIED from a real photo, not invented from text — so an official jersey
// comes out as the real jersey, not an approximation.
//
// Model is env-configurable so it can be swapped WITHOUT a redeploy if the
// first choice doesn't preserve crests/logos well enough:
//   REPLICATE_TRYON_MODEL = "owner/name:VERSION_HASH"
// Default points to a known virtual try-on model. PIN THE VERSION in Vercel
// (copy owner/name:hash from the model's API tab) to avoid 404s.
const TRYON_MODEL =
  process.env.REPLICATE_TRYON_MODEL ?? "cuuupid/idm-vton";

// Field names vary by model. Defaults match IDM-VTON-style inputs
// (human_img + garm_img + a short garment description). Override via env if you
// pin a model with different field names:
//   REPLICATE_TRYON_PERSON_FIELD  (default "human_img")
//   REPLICATE_TRYON_GARMENT_FIELD (default "garm_img")
//   REPLICATE_TRYON_DESC_FIELD    (default "garment_des")
const PERSON_FIELD = process.env.REPLICATE_TRYON_PERSON_FIELD ?? "human_img";
const GARMENT_FIELD = process.env.REPLICATE_TRYON_GARMENT_FIELD ?? "garm_img";
const DESC_FIELD = process.env.REPLICATE_TRYON_DESC_FIELD ?? "garment_des";

export async function tryOnGarment(input: {
  personUrl: string;
  garmentUrl: string;
  garmentDescription?: string;
}): Promise<string> {
  const fields: Record<string, unknown> = {
    [PERSON_FIELD]: input.personUrl,
    [GARMENT_FIELD]: input.garmentUrl,
  };
  // Many try-on models accept (and improve with) a short garment description.
  if (DESC_FIELD) {
    fields[DESC_FIELD] = input.garmentDescription?.slice(0, 200) || "garment";
  }

  const reqBody: Record<string, unknown> = { input: fields };

  const colon = TRYON_MODEL.indexOf(":");
  let url: string;
  if (colon >= 0) {
    url = `${API_BASE}/predictions`;
    reqBody.version = TRYON_MODEL.slice(colon + 1);
  } else {
    url = `${API_BASE}/models/${TRYON_MODEL}/predictions`;
  }
  ...
  if (!res.ok) {
    const detail = await res.text().catch(() => "");
    console.error(
      `[tryon] submit failed: status=${res.status} model="${TRYON_MODEL}" detail=${detail || res.statusText}`
    );
    throw new Error(`Virtual try-on failed (${res.status}): ${detail || res.statusText}`);
  }
  ...
  for (let i = 0; i < 40; i++) {
    if (pred.status === "succeeded") {
      const out = firstUrl(pred.output);
      if (!out) throw new Error("Virtual try-on returned no output.");
      return out;
    }
    if (pred.status === "failed" || pred.status === "canceled") {
      throw new Error(`Virtual try-on ${pred.status}${pred.error ? `: ${pred.error}` : ""}`);
    }
    await sleep(2000);
    const poll = await fetch(`${API_BASE}/predictions/${pred.id}`, {
      headers: { Authorization: `Bearer ${getToken()}` },
    ...
    if (!poll.ok) {
      const detail = await poll.text().catch(() => "");
      throw new Error(`Virtual try-on poll failed (${poll.status}): ${detail || poll.statusText}`);
    }
    pred = await poll.json();
  }
